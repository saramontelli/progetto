...
Boid::Boid(Vector pos, Vector vel, bool predator) : pos_(pos), vel_(vel), predator_(predator) {}
...
Vector Boid::separation(const std::vector<Boid>& near, float s,
                        float d_s, float x_max, float y_max) const {
  Vector v_1(0., 0.);

  for (const auto& boid : near) {
  float dx = pos_.get_x() - boid.get_pos().get_x();
    float dy = pos_.get_y() - boid.get_pos().get_y();

    if (dx > x_max / 2) dx -= x_max;
    else if (dx < -x_max / 2) dx += x_max;

    if (dy > y_max / 2) dy -= y_max;
    else if (dy < -y_max / 2) dy += y_max;

   float dist = std::sqrt(dx*dx + dy*dy);

   if (dist < d_s && dist > 0.01f) {
      v_1 += Vector(dx, dy) * (s / dist*dist); // Forza inversamente proporzionale
    }
  }
  return v_1;
  }
...
Vector Boid::cohesion(const std::vector<Boid>& near, float c, float x_max, float y_max) const {
  ...
  Vector avg_relative_pos = relative_sum * (1.f / static_cast<float>(near.size()));
